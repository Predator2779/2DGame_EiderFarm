...
public class PathFinder2 : MonoBehaviour
{
    ...
    private List<Node> CheckedNodes = new();
    private List<Node> WaitingNodes = new();
    ...
    {
        Vector2 StartPosition = new Vector2(Mathf.Round(_currentPos.x), Mathf.Round(_currentPos.y));
        Vector2 TargetPosition = new Vector2(Mathf.Round(_targetPos.x), Mathf.Round(_targetPos.y));
        
        Node startNode = new Node(0, StartPosition, TargetPosition, null);
        
        return SearchInDepth(startNode);
    }
    
    private List<Vector2> MyAlgorythm()
    {
